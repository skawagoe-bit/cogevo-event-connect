...
import { scanBusinessCard } from "@/app/actions/sansan";
...
export default function ScanPage() {
  ...
  const handleAnalyzeCard = async () => {
    ...
    try {
        const result = await scanBusinessCard(capturedImage);
        ...
        if (result.success && result.data) {
            ...
            throw new Error(result.error || "データ化に失敗しました");
        }
    } catch (e: any) {
        console.error(e);
      ...
      <header className="bg-white border-b p-3 flex justify-between items-center shadow-sm z-20 shrink-0">
         <div className="flex flex-col">
            <div className="flex items-center gap-2">
                ...
                <span className="bg-emerald-600 text-white text-[10px] px-1.5 py-0.5 rounded font-bold">v2.3</span>
            </div>
